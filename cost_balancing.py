...
def main_warthog5():
    HighCostSampleNum = 9000
    HighCostTrainNum = 8000
    LowCostSampleNum = 4000
    LowCostTrainNum = 3000
    ...
    targetfolder = '/project/learningphysics/tartancost_arl_20220922/'
    select_traj, select_costsind, high_costlist = find_highcost_frames(sort1, rootdir, trajdirs, 
                                                    target_frame_number = HighCostSampleNum, 
                                                    avoid_consecutive_num = 2, cost_thresh=0.1, 
                                                    odom_len=OdomLen, savefilename = "select_highcost_arl_20220922")
    train_traj, test_traj, train_costsind, test_costsind, train_costlist, test_costlist = split_train_val(select_traj, select_costsind, high_costlist, HighCostTrainNum)
    copy_data(targetfolder+highoutputfolder, rootdir, train_traj, train_costsind, train_costlist, odom_len= OdomLen)
    copy_data(targetfolder+highoutputfolder_val, rootdir, test_traj, test_costsind, test_costlist, odom_len= OdomLen)
    ...
    select_traj, select_costsind, low_costlist = find_lowcost_frames(sort1, rootdir, trajdirs, 
                                                    ...
                                                    odom_len=OdomLen, savefilename = "select_lowcost_arl_20220922")
    train_traj, test_traj, train_costsind, test_costsind, train_costlist, test_costlist = split_train_val(select_traj, select_costsind, low_costlist, LowCostTrainNum)
    copy_data(targetfolder+lowoutputfolder, rootdir, train_traj, train_costsind, train_costlist, odom_len= OdomLen)
    copy_data(targetfolder+lowoutputfolder_val, rootdir, test_traj, test_costsind, test_costlist, odom_len= OdomLen)
...
def cost_statistics2(rootdir1, rootdir2, savefile):
    trajdirs = listdir(rootdir1)
    trajdirs = [tt for tt in trajdirs if isdir(join(rootdir1, tt))]
    trajdirs.sort()
    trajnum = len(trajdirs)
    costlist1 = []
    for traj in trajdirs:
        costs = load_costs(rootdir1 + '/' + traj, costfilename='cost.npy', costfolder='cost2')
        costlist1.extend(costs.tolist())

    trajdirs = listdir(rootdir2)
    trajdirs = [tt for tt in trajdirs if isdir(join(rootdir2, tt))]
    trajdirs.sort()
    trajnum = len(trajdirs)
    costlist2 = []
    for traj in trajdirs:
        costs = load_costs(rootdir2 + '/' + traj, costfilename='cost.npy', costfolder='cost2')
        costlist2.extend(costs.tolist())

    plt.hist(costlist1, bins=100, log=True, color=(0,0,1,0.5))
    plt.hist(costlist2, bins=100, log=True, color=(1,0,0,0.5))
    plt.legend(['TartanDrvie', 'New2022'])
    plt.savefig(savefile)
...
if __name__ == '__main__':
    ...
    # rootdir = '/project/learningphysics/arl_20220922_traj'
    # savefile = 'costs2_arl_20220922.png'
    # cost_statistics(rootdir, savefile)
    # import ipdb;ipdb.set_trace()
    # test_main()
    # main_tartandrive()
    # cost_statistics2('/project/learningphysics/tartandrive_trajs','/project/learningphysics/2022_traj', 'two_hist.png')
    main_warthog5()