import numpy as np
from .transformation import pos_quats2SE_matrices, SE2quat, pose2motion, motion2pose

def check_quat_valid(traj):
    for t in traj:
        quat = t[3:]
        if abs(np.sum(quat*quat) -1 )> 1e-6:
            print(abs(np.sum(quat*quat) -1 ))
            return False
    return True
