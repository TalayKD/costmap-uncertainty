import numpy as np
from .trajectory_transform import trajectory_transform, rescale, check_quat_valid
from .transformation import pos_quats2SE_matrices, SE2quat, SEs2quats

np.set_printoptions(suppress=True, precision=2, threshold=100000)

def transform_trajs(gt_traj, est_traj, cal_scale):
    if not check_quat_valid(est_traj):
        raise Exception("QUATERNION_FORMAT_ILLEGAL: Quaternion should be normalized! ")
...
#     def evaluate(self, gt_traj, est_traj):
...
class ATEEvaluator(object):
    ...
    def evaluate(self, gt_traj, est_traj, scale):
        ...
        T[:3,3:] = trans 
        T = np.linalg.inv(T)
        est_traj_aligned = np.matmul(T, np.array(est_SEs))
        est_traj_aligned = est_traj_aligned.reshape(est_traj_aligned.shape[0],-1)
        est_traj_aligned = est_traj_aligned[:,:12]
        est_traj_aligned = SEs2quats(est_traj_aligned)

        # est_traj_aligned = []
        # for se in est_SEs:
        #     se[:3,3] = se[:3,3] * s
        #     se_new = T.dot(se)
        #     se_new = SE2quat(se_new)
